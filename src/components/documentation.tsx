...
export default function Documentation({ lang }: DocumentationProps) {
    ...
    const loadMarkdown = async () => {
      ...
      try {
        const response = await fetch(
          `${import.meta.env.BASE_URL}docs/${lang}/documentation.md`
        );
        if (!response.ok) {
      ...
      <article className="prose prose-lg dark:prose-invert max-w-4xl mx-auto">
        <ReactMarkdown
          ...
          components={{
            ...
            th: ({ children }) => (
              <th className="border border-gray-300 px-4 py-2 bg-gray-100 dark:bg-gray-800 font-semibold text-left">
                {children}
              </th>
            ),
            td: ({ children }) => (
